...
function updateEcolesSelection() {
    ...
    const nbRequired = nbEcolesEl ? safeParseInt(nbEcolesEl.value, 1, 1) : 1;
...
function validateForm() {
    ...
    const dureeFormation = dureeFormationValEl ? dureeFormationValEl.value : '';
    if (!dureeFormation) {
    ...
    const dispositifGAIA = dispositifGAIAEl ? validateInput(dispositifGAIAEl.value.trim(), 10) : '';
    ...
    const moduleGAIA = moduleGAIAEl ? validateInput(moduleGAIAEl.value.trim(), 5) : '';
    ...
    const numeroGroupe = numeroGroupeEl ? numeroGroupeEl.value.trim() : '';
    if (numeroGroupe) {
...
async function validerFormulaire() {
    ...
    const numeroGroupeEl = document.getElementById('numeroGroupe');
    const numeroGroupeRaw = numeroGroupeEl ? validateInput(numeroGroupeEl.value.trim(), 10) : '';
    ...
    const dureeFormation = dureeFormationEl ? dureeFormationEl.value : '';
    const modalitesFormation = getCheckboxValues('modalitesFormation');
    const objetsTransversaux = getCheckboxValues('objetsTransversaux');
    const themesFormation = getCheckboxValues('themesFormation');
    const anneeScolaireEl = document.getElementById('anneeScolaire');
